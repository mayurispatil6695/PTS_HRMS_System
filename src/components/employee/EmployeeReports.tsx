...
import { generateReportData, exportReport, ReportData, SalaryReportData } from '../../utils/reportUtils';
...
const EmployeeReports = () => {
  ...
  useEffect(() => {
    const data = generateReportData(reportType, dateRange, user);
    setReportData(data);
  ...
  const renderReportSummary = () => {
    switch (reportType) {
      case 'attendance':
        return <AttendanceReportSummary employeeId={user?.id} />;
      case 'salary':
        // Cast to SalaryReportData when rendering salary summary
        return <SalaryReportSummary reportData={reportData as SalaryReportData} />;
      case 'projects':
        return <ProjectsReportSummary adminUid={user?.adminUid} />;
      case 'leaves':
        return <LeaveReportSummary employeeId={user?.id} />;
      case 'tasks':
        return <DailyTask />;
      default:
        return null;
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {reportType === 'tasks' ? 'Daily Tasks' : reportType.charAt(0).toUpperCase() + reportType.slice(1) + ' Summary'}
            </CardTitle>
          </CardHeader>
          <CardContent>{renderReportSummary()}</CardContent>
        </Card>
      </motion.div>
    </div>
  );
